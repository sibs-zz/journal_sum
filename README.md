...
单次运行的日期窗口是 15 天，含当天。更早的论文不会进入本轮。

## 关注什么

筛选标准与原先脚本一致，仍然从作物研究是否受启发来打分：
...
新闻、社论、更正、招聘广告、培训和会议通知会被丢掉。分数达到阈值的文章才生成中文总结。
...
公众号不限于上面六个期刊，用来补中文解读和其他植物、生命科学来源。目前跟踪四个栏目：植物科学最前沿、BioArt植物、BioArt、iPlants。同样只取最近半个月。
...
| `DEEPSEEK_MODEL` | 模型 | deepseek-v4-pro |
| `DEEPSEEK_MAX_TOKENS` | 单次输出上限 | 8192 |
| `MIN_SCORE` | 保留阈值（0–10） | 7 |
| `MAX_KEEP_PER_SOURCE` | 每个来源最多总结篇数 | 12 |
